import fs from 'node:fs';import path from 'node:path';import {createRequire} from 'node:module';import {pathToFileURL} from 'node:url';import {spawnSync} from 'node:child_process';
...
const result=r.finish();result.typeCommand={status:type.status,command:[process.execPath,c.tsc,...args]};fs.writeFileSync(c.output,JSON.stringify(result,null,2),{flag:'wx'});console.log(result.summary);if(result.summary.failed)process.exitCode=1;
