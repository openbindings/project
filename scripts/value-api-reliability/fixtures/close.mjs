...
await report.observe('CLOSE-02.5',({check,equal})=>{equal(packages.publication,false);for(const s of Object.values(selection.sources))check(git(s.dir,'branch','--show-current').startsWith('codex/value-api-reliability-'));for(const file of fs.readdirSync(path.join(c.root,'commands')).filter(x=>x.endsWith('.json'))){const r=read(path.join(c.root,'commands',file));check(!r.command.some(x=>['push','publish','deploy','promotion','merge'].includes(x)));}});
