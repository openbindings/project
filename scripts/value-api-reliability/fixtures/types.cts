...
export async function run(){const expression:jsonata.Expression=jsonata('$sum(prices)');const result=await expression.evaluate<number>({prices:[1,2]});if(result!==3||engine.representation!=='value'||error.code!=='T2009')throw Error('CJS expectation');const x=await jsonata('$').evaluate<json.Value>(json.number('0.1'));if(x===undefined||json.stringify(x)!=='0.1')throw Error('CJS exact value');return true;}
if(false){
